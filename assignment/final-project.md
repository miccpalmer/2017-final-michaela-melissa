...
To solve for the missing data issue in March 2017, we tried to forecast values based on 2015 and 2016 values. Forecasts have been generated by applying `forecast()` directly to each time series. This will select an ETS model using the AIC, estimate the parameters, & generate forecasts. Although it returns prediction intervals, we’ve extracted the point forecasts. We are using mean in the returned forecast object because they are usually the mean of the forecast distribution.
