...
To solve for the missing data issue, we are going to generate randome values from the min and max of generation of the other spring months values, April and May. 
```{r}
# i cant get the last function to work SOS CARL AND DANA
...
vals <- function(source){
  runif(600, min = mins$source, max = maxs$source) 
}
...
runif(600, min = mins$biogas, max = maxs$biogas) #it works manually
...
#march <- data.frame(march, col_names = c("date", "hour", "geothermal", "biomass", "biogas", "small_hydro", "wind", "solar_pv", "solar_thermal"))
